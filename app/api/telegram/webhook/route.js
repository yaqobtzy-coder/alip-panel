...
const CANVAS_TOOLS = {};
async function findUserByUsername() { return null; }
async function adjustUserSaldo() { return null; }
...
async function handleMenuAction(key, cq, chatId) {
  ...
  if (key === "saldo") {
    await setPendingInput(chatId, { kind: "saldo_username" });
    await askForReply(chatId, "💰 Balas dengan *username* user toko (Rayy Store) yang mau di-set saldo-nya.");
    return NextResponse.json({ ok: true });
  }
...
// Toggle ON/OFF gateway pembayaran (AustinPay) dari tombol di menu
// "💳 AustinPay", lalu edit pesan yang sama biar keliatan status
// terbarunya tanpa spam pesan baru.
async function handlePaymentGatewayToggle(gateway, cq, chatId) {
  if (gateway !== "austinpay") {
    await answerCallback(cq.id, "Gateway tidak dikenal.");
    return NextResponse.json({ ok: true });
  }

  const current = await getSiteConfig();
  const nextEnabled = !(current.paymentGateways?.austinpay !== false);
  const updated = await setPaymentGatewayEnabled(gateway, nextEnabled);

  await answerCallback(cq.id, `AustinPay sekarang ${nextEnabled ? "ON ✅" : "OFF 🔴"}.`);
...
async function handleMessage(message) {
  ...
  // --- Set Saldo User ---
  if (kind === "saldo_username") {
    const uname = text.trim();
    if (!uname) {
      await sendOwnerText(chatId, "Username tidak boleh kosong. Balas lagi.");
      return NextResponse.json({ ok: true });
    }
    ...
    if (!user) {
      await sendOwnerText(chatId, `❌ User \`${uname}\` gak ketemu di Rayy Store. Balas lagi dengan username yang benar.`);
      return NextResponse.json({ ok: true });
    }
    await setPendingInput(chatId, { kind: "saldo_amount", data: { uid: user.id, username: user.username } });
    await askForReply(
      chatId,
      `💰 Saldo \`${user.username}\` sekarang *Rp${Number(user.saldo || 0).toLocaleString("id-ID")}*.\n\nBalas dengan jumlah yang mau *ditambahkan* (mis. \`50000\`), atau pakai minus buat *mengurangi* (mis. \`-20000\`).`
    );
    return NextResponse.json({ ok: true });
  }

  if (kind === "saldo_amount") {
    const delta = Math.trunc(Number(text.replace(/[^0-9-]/g, "")));
    if (!Number.isFinite(delta) || delta === 0) {
      await sendOwnerText(chatId, "Jumlah tidak valid. Balas lagi dengan angka, mis. `50000` atau `-20000`.");
      return NextResponse.json({ ok: true });
    }
    await clearPendingInput(chatId);
    try {
      const newBalance = await adjustUserSaldo(data.uid, delta, {
        type: "topup_admin",
        note: "Diset lewat bot Telegram",
        actor: "telegram"
      });
      await sendOwnerText(
        chatId,
        `✅ Saldo \`${data.username}\` ${delta > 0 ? "ditambah" : "dikurangi"} Rp${Math.abs(delta).toLocaleString("id-ID")}.\nSaldo sekarang: *Rp${newBalance.toLocaleString("id-ID")}*.`
      );
    } catch (err) {
      await sendOwnerText(chatId, `❌ ${err.message || "Gagal mengubah saldo."}`);
    }
    return NextResponse.json({ ok: true });
  }
