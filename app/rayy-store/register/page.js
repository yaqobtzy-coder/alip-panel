...
export default function RayyRegisterPage() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/rayy-store/register", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-10">
      <div className="card w-full max-w-sm p-6 hover-lift" data-reveal="scale">
        <h1 className="display text-xl font-bold text-white mb-1">Daftar Akun</h1>
        <p className="text-sm text-muted mb-6">Buat akun Rayy Store buat mulai belanja.</p>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            <label className="text-xs text-muted">Nomor WhatsApp</label>
            <input
              className="field mt-1"
              value={form.whatsapp}
