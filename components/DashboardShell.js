...
export default function DashboardShell({ expectedRole }) {
  ...
  // Session + site-config paralel biar owner dashboard lebih cepet
  useEffect(() => {
    let cancelled = false;

    // site-config tidak tergantung login — jalanin bareng
    ...
    (async () => {
      try {
        ...
        setSession(data);
        // Data sekunder paralel — jangan nahan render
        if (data.role === "pt" || data.role === "owner") {
          loadAccounts();
        }
        if (["fullup", "reseller", "pt"].includes(data.role)) {
          loadPendingNotice();
        }
      } catch {
  ...
  if (!session) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-3 text-muted px-4">
        <i className="fa-solid fa-spinner fa-spin text-2xl text-accent" />
        <p className="text-sm">Memuat dashboard…</p>
        <p className="text-xs text-muted/80 text-center max-w-xs">
          Kalau lama, biasanya cold start server. Tunggu sebentar.
        </p>
      </main>
    );
  }
